...
bool B1SECS2ClientSession::sendMessageS5F5(const B1SECS2DataALID& alID)
{
    B1SECS2MessageS5F5Writable message(alID);
    if (message.writeData() != true) {
        return false;
    }
    sendDataMessage(_secs2SessionID, makeNewSystemBytes(), message);
...
bool B1SECS2ClientSession::sendMessageS5F7()
{
    B1SECS2MessageS5F7Writable message;
    if (message.writeData() != true) {
        return false;
    }
    sendDataMessage(_secs2SessionID, makeNewSystemBytes(), message);
